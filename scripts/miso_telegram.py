...
if __name__ == "__main__":
  ...
  python3 miso_telegram.py send <chat_id> <phase> [caption]
  python3 miso_telegram.py edit <chat_id> <message_id> <phase> [caption]
...
Phases: init, running, partial, approval, complete, error"""
    ...
    if action == "pin":
        ...
        result = send_animation(chat_id, phase, caption)
    elif action == "edit":
        chat_id = int(sys.argv[2])
        message_id = int(sys.argv[3])
        phase = sys.argv[4]
        caption = sys.argv[5] if len(sys.argv) > 5 else ""
        result = edit_message_media(chat_id, message_id, phase, caption)
    else:
        print(usage)
        sys.exit(1)
